...
class OAKKernel(gpflow.kernels.Kernel):
    ...
    def __init__(
        ...
        if spatial_block_idx is not None:
            ...
            self.kernels = [
                k for i, k in enumerate(self.kernels) if i != spatial_block_idx
            ]
            # change active_dims
...
class KernelComponenent(gpflow.kernels.Kernel):
    ...
    def K(self, X, X2=None):
        ...
        var_n = (
            self.oak_kernel.variances[n_order] if self.share_var_across_orders else 1
        )
...
def get_list_representation(
    ...
    for order in range(1, kernel.max_interaction_depth + 1):
        for raw_combo in itertools.combinations(range(num_dims), order):
            ...
            kernel_list.append(
                KernelComponenent(kernel, kcombo, share_var_across_orders)
            )
    return selected_dims, kernel_list
